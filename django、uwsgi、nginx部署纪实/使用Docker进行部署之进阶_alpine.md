...
#
# 1、配置清华镜像地址
# 2、安装时区包并配置时区TIME_ZONE为中国标准时间
# 3、更新升级软件
# 4、安装软件python3、nginx、 supervisor,升级pip,setuptools,安装 uwsgi
RUN echo "https://mirror.tuna.tsinghua.edu.cn/alpine/v3.8/main/" > /etc/apk/repositories \
    && apk add --no-cache -U tzdata \
    && ln -sf /usr/share/zoneinfo/${TIME_ZONE} /etc/localtime \ 
    && echo "${TIME_ZONE}" > /etc/timezone  \
    && apk add --update --upgrade \    	
	&& apk add --no-cache vim python3 gcc make libc-dev linux-headers pcre-dev python3-dev nginx supervisor mariadb-dev \    
    && python3 -m ensurepip \
